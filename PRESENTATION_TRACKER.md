...
- [x] ✅ **Reconciled the appendix instability slides (2026-07-13).** Rewrote `phase3/bootstrap.py` to resample the A/B trials through the locked Bradley-Terry pipeline (point estimates match `score.py` to 4 dp); regenerated `results/bootstrap_ci.csv` and the three CI figures via new `phase3/plot_instability_ci.py`. Result overturned the old headline: **13 of 90** clear the noise floor (not 6); the ordinary adult survives in **0/3** models; true all-model survivors are **gpt56sol + human_newborn**. Per user decision, **cut the "Most Incoherent Case" slide entirely**, updated "Instability — Now With Error Bars" to 13/90 with new figures, and trimmed the false "ordinary adult" clause from the main-deck story card. JA dictionary entries updated/removed to match. *Note: locked BT scoring still covers only the 3 confirmatory models; run-2/Claude/Gemini BT extension remains open (audit Table 5).*
