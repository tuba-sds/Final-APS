...
Six of eight point estimates fall below 2 — but the robustness battery (§4.4) sharpens what this table can and cannot say, and we state H2 in its defensible form: **for no model can the four-construct structure be distinguished from a single care factor plus measurement noise** — every slider sits at its own one-factor-null median, only Claude's below-2 value is certain under resampling, and the per-model ordering carries no information (a rep-count confound compounds this, §4.4). "Which model is most collapsed" is therefore not a claim this instrument can make; "nothing here requires more than one care factor" is. The two above-2 point estimates are DeepSeek-R1 (2.21) and Llama 4 Scout (2.27) — notably *not* a reasoning-model story, since Qwen 3 32B, the cohort's reasoning model, still lands at 1.79. Both exceptions should be read with caution: these are the cohort's two most position-biased models (Scout picks option "A" 96% of the time, DeepSeek-R1 91% — position bias is a documented LLM pairwise-judgment artifact; Zheng et al., 2023), and in the session-memory extension — where seeing their own prior choices dissolves that bias — both care sliders fall below the threshold (Scout 2.27 → 1.81, DeepSeek-R1 2.26 → 1.50; see the memory appendix). Part of their apparent extra dimensionality is thus positional noise, not moral structure.
