...
Six of eight point estimates fall below 2 — and the robustness battery (§4.4) turns this into a sharper, positive claim than the point estimates alone could carry: **there really is one care factor.** Three pieces lock together. The one-factor null: every slider sits at its own one-factor-plus-noise median — nothing in any model's data requires a second factor. The format diversification: the collapse is not an artifact of the A/B question format — it survives five answer formats (§4.4). And the uncertainty analysis: only Claude's below-2 value is certain under resampling, and the per-model ordering carries no information (a rep-count confound compounds this, §4.4) — so "which model is most collapsed" is not a claim this instrument can make, while "one care factor" is. The two above-2 point estimates are DeepSeek-R1 (2.21) and Llama 4 Scout (2.27) — notably *not* a reasoning-model story, since Qwen 3 32B, the cohort's reasoning model, still lands at 1.79. Both exceptions should be read with caution: these are the cohort's two most position-biased models (Scout picks option "A" 96% of the time, DeepSeek-R1 91% — position bias is a documented LLM pairwise-judgment artifact; Zheng et al., 2023), and in the session-memory extension — where seeing their own prior choices dissolves that bias — both care sliders fall below the threshold (Scout 2.27 → 1.81, DeepSeek-R1 2.26 → 1.50; see the memory appendix). Part of their apparent extra dimensionality is thus positional noise, not moral structure.
...
**No slider is a parse artifact (all models × all parse policies).** The scores rest on parse decisions — the D9 strict rule, the D21 verbal-vote fallback, refusals dropped rather than scored, and for Claude (which declined the binary in 756 of 921 judged replies) a judged-refusal pipeline — so every model's slider was recomputed under every defensible alternative: strict-parse-only with declines dropped, declines counted as *ties* (half a win to each side), the committed pipeline, and — for the two frontier models — judged refusal leans counted as votes (`parse_policy_all.csv`; Figure 4). **Across all models and all policies there is not one threshold crossing**; the largest movement anywhere is 0.09 (Llama 3.1 under ties, 1.70 → 1.79), Claude stays within 1.30–1.33 (its per-entity scores correlate ρ ≥ 0.99 with the committed pipeline under every policy, `parse_sensitivity_claude.csv`), and the roster statement survives excluding Claude entirely (5 of the remaining 7 below 2). The evaluator's parse choices, in this design, do not move the conclusion.

![Figure 4](figures/phase3_parse_policy.png)

**Figure 4. The care slider under every parse policy, all models.** Dot = strict parse with declines dropped; square = the committed pipeline; triangle = declines as ties; open diamond = judged refusal leans as votes (frontier models). Where only the square is visible, the policies coincide exactly — which is the finding. Dashed line = the declared threshold of 2.
