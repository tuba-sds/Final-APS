...
**Six of eight AI models fall below 2**, so **H2 is supported**. The two exceptions are DeepSeek-R1 (2.21) and Llama 4 Scout (2.27) — and this is notably *not* a reasoning-model story, since Qwen 3 32B, the cohort's reasoning model, still falls at 1.79. Scout's exception should be read with caution: it is the cohort's most position-biased model (it picks option "A" 96% of the time), and in the session-memory extension — where seeing its own prior choices dissolves that bias — its care slider falls to 1.81, below the threshold (see the memory appendix). Part of its apparent extra dimensionality is thus positional noise, not moral structure.
...
- **Human baseline is a convenience sample.** N = 32 meets the preregistered minimum of 30, but the raters are mostly Japan-based volunteers and items were skippable (per-item n = 6–32) — so every AI-vs-human (H1) result here is exploratory (D13/D20).
...
- **Is refusal a moral-uncertainty signal?** On the 20 shared dilemmas the *placement* of AI declines tracks human reticence: item-level decline rates correlate at Spearman r = 0.53 (p = .017), three of the top-five decline items coincide (girl-vs-boy, girl-vs-pregnant, dog-vs-pig), and on 9 of 20 items both sides decline at <5% — agreement at the floor as well as at the peak. But the signal is fragile and gated: the correlation weakens to r = 0.50 (p = .029) without the AI's policy-flavored triage outlier, r = 0.45 (p = .052) without the top shared item, and r = 0.40 (p = .099) without both; and declining is vendor-concentrated (Claude 20.8% on these items; four of eight models, 0%). Read as a detector: where refusal fires, it lands on genuinely contested ground (high precision), but most models never fire it (low coverage) — refusal is a vendor-policy-mediated uncertainty signal, not a general one. The direct test — an A/B/"prefer not" re-run giving the models the same opt-out humans had — remains to be run.
- **Does session memory destabilize moral judgment?** The full battery was re-run with session memory for all four open run-2-era models (`phase3-with-memory/`): Qwen 3 and DeepSeek-R1 with full transcripts, gpt-oss-20b and Llama 4 Scout with a programmatic summary of their previous choices (the summary-memory variant, marked * throughout). Two effects replicate across all four: position bias evaporates (DeepSeek 91% → 50% A-picks, Scout 96% → 69%) and order-consistency rises (+11 to +53 points; Qwen 83% → 95%, Scout 8% → 61%). Memory also re-frames two baseline results. Scout's seemingly excellent consistency (same answer every rep, 91%) was a position-bias artifact — with memory its care slider falls 2.28 → 1.81, dissolving one of H2's two exceptions — and DeepSeek's collapse (84% → 27%) is confounded with sheer output breakdown (unusable replies 7% → 87% across a 300-turn transcript), which the summary variant avoids entirely (Scout: 0% unusable). Tentative answer: memory *stabilizes* the judgments themselves (anchoring) while long raw transcripts destabilize *output formatting* in fragile models — a deployment-relevant distinction: an agent's turn-250 judgments differ from its turn-1 judgments mostly when context handling breaks, not because its values drift.

## Data and code availability

The full pipeline (config-driven runner for all 8 AI models, Bradley-Terry scorer, dimensionality, Mantel tests, bootstrap CIs, refusal tooling) and all raw and derived data are in the project repository; `./run_all.sh` regenerates every derived output from the committed raw data (the preregistered `rsa_results.csv` is deliberately kept frozen as executed; the bootstrap runs behind `--with-bootstrap` because its CI bounds jitter across reruns; `--collect` re-runs the model batteries themselves). Key files per claim: dimensionality — `results/derived/dimensionality_all.csv` (human counterpart: `human_dimensionality.csv`); ratings match — `rsa_comparison_results.csv` (preregistered path: `rsa_results.csv`; all-8 exploratory extension: `rsa_results_all8.csv`); clusters — `clusters_all8.csv`; all-8 noise floor — `bootstrap_ci_all8.csv`; choices match — `human_vs_ai_comparison.csv`, `human_ai_forcedchoice_agreement.csv`; care ladder — `care_ladder_all8.csv`, `human_care_index.csv`; bootstrap — `bootstrap_ci.csv`; refusals — `refusals.csv`, `claude_refusal_polarity.csv`; caution vs human reticence — `caution_comparison.csv`. The live demo ("You vs. the AI") runs the identical battery at [aps-dashboard-0dmj.onrender.com](https://aps-dashboard-0dmj.onrender.com).
