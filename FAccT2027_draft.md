...
We report the descriptive Campbell–Fiske triad, and back it with a formal **correlated-uniqueness MTMM confirmatory factor analysis** pooled over the six fully-measured open-and-Claude batteries (entity × model rows, 160 complete cases, 4 traits × 5 methods; method variance modeled as correlated residuals within a method — the CU variant, since correlated-methods models are notorious non-convergers). The CFA sharpens the claim rather than merely confirming it: a strict one-factor model is rejected against four correlated traits (Δχ²(6) = 526, CFI 0.709 → 0.886), **but the four traits resolve into one tight care cluster and one partial outlier**: empathy–protectiveness correlate at **0.94** and sentience–empathy at **0.86** at the latent level, while **agency is only partially separable** (r = 0.27–0.41 with the others). Mean trait-variance share per item is 0.58 against a mean method-residual correlation of 0.12 — trait variance dominates, method variance is real but modest. This is exactly what per-model dimensionality estimates between 1 and 2 imply: one dominant care axis carrying sentience, empathy and protectiveness nearly collinearly, with agency contributing the fraction of a second dimension. (Fit is moderate — RMSEA 0.125, n = 160 — so we treat the CFA as structure identification under pooled power, not a precision instrument.) *Convergent validity:* the same construct measured by different formats correlates well where models are reliable (mean same-construct cross-format r: Claude 0.80, Gemini 0.72, Qwen 3 0.69, Scout 0.64, Gemma 2 0.64). *The decisive comparison:* if the collapse were method variance, inter-construct correlations would crash when the two constructs come from **different** formats. They do not: heteromethod inter-construct correlations run 0.24–0.66, and for Claude they are essentially identical to the within-format value (0.66 vs 0.68). *Dimensionality on method-averaged scores* — which averages out method-specific and noise variance — keeps six of seven fully-measured models below 2 (Claude 1.56), and DeepSeek-R1, above threshold on the single format, drops to 1.69 once method noise averages out, converging with the reliability account above. **Table 2. The descriptive multitrait–multimethod triad** (mean correlations across the five methods; Gemini partial, four methods).
...
4. CU-MTMM CFA: DONE (cu_mtmm_cfa.csv) — folded into Finding 1.
